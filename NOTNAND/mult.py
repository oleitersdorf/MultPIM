...
def MultPIM(crossbar: Crossbar, N: int):
    ...
    # --- 2 OPs --- #
    crossbar.perform(
        Operation([Gate(GateType.INIT1, [], [(1, TEMP1), (1, ABIT), (1, BBIT), (1, 3), (1, 4), (1, 5)])] + [Gate(GateType.INIT1, [],
    ...
    for k in range(2 * N):
        ...
        if k < N:
            ...
            # TEMP1 = NAND(ABBIT if is_notted[j] else BBIT, SBIT) = NAND(A, B)
            # TEMP2 = NAND(ABBIT if is_notted[j] else BBIT, TEMP1) = NAND(A, NAND(A, B))
            # TEMP3 = NAND(TEMP1, SBIT) = NAND(NAND(A, B), B)
            # INIT(ABBIT, SBIT)

            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, ABBIT if is_notted[j] else BBIT), (j, SBIT)], [(j, TEMP1)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, ABBIT if is_notted[j] else BBIT), (j, TEMP1)], [(j, TEMP2)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, TEMP1), (j, SBIT)], [(j, TEMP3)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
            ...
            # ABBIT = NAND(TEMP2, TEMP3)
            # SBIT = NAND(ABBIT, CBIT)
            # INIT(TEMP2, TEMP3)

            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, TEMP2), (j, TEMP3)], [(j, ABBIT)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, ABBIT), (j, CBIT)], [(j, SBIT)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
            ...
            # TEMP2 = NAND(ABBIT, SBIT)
            # TEMP3 = NAND(SBIT, CBIT)
            # INIT(CBIT)

            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, ABBIT), (j, SBIT)], [(j, TEMP2)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, SBIT), (j, CBIT)], [(j, TEMP3)])
                for j in range(2, N + 1)
            ] + [Gate(GateType.MAGIC_NOT, [(1, ABIT)], [(1, 3)])]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.INIT1, [], [(j, CBIT)])
                for j in range(2, N + 1)
            ] + [Gate(GateType.MAGIC_NOT, [(1, BBIT)], [(1, 4)])]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, TEMP1), (j, SBIT)], [(j, CBIT)])
                for j in range(2, N + 1)
            ] + [Gate(GateType.FELIX_NAND, [(1, 3), (1, 4)], [(1, 5)])]))

            # INIT(SBIT)
            # SBIT = NAND(TEMP2, TEMP3)
            ...
            # --- 2 OPs --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, TEMP2), (j, TEMP3)], [(j + 1, SBIT)])
                for j in range(3, N + 1, 2)
            ] + [Gate(GateType.MAGIC_NOT, [(1, 5)], [(2, SBIT)]) if k < N else Gate(GateType.INIT0, [], [(2, SBIT)])]))
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, TEMP2), (j, TEMP3)], [(j + 1, SBIT)])
                for j in range(2, N, 2)
            ] + [Gate(GateType.FELIX_NAND, [(N, TEMP2), (N, TEMP3)], [(N + 1, k)])]))

        else:

            # TEMP1 = NAND(SBIT, CBIT)
            # TEMP2 = NAND(SBIT, TEMP1)
            # TEMP3 = NAND(CBIT, TEMP1)
            # INIT(CBIT)

            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, SBIT), (j, CBIT)], [(j, TEMP1)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, SBIT), (j, TEMP1)], [(j, TEMP2)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.FELIX_NAND, [(j, CBIT), (j, TEMP1)], [(j, TEMP3)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
        ...
        # --- 1 OP --- #
        crossbar.perform(Operation([Gate(GateType.INIT1, [], [(1, BBIT), (1, 3), (1, 4), (1, 5)])] + [
